...
export class PlotDataService {
  ...
  getDataFiles(names: string[]): Observable<DataFile[]> {
    const fetches = this.dataFiles
      .filter((file) => names.includes(file.name))
      .map((file) =>
        this.http.get<any>(`${this.url}/${file.file}`).pipe(
          map((data) => {
            file.content = data;
            return file;
          }),
  ...
  getFileList(): Observable<string[]> {
    return this.http.get<DataFile[]>(`${this.url}/list.json`).pipe(
      tap((response) => (this.dataFiles = response)),
      map((response) => response.map((item) => item.name)),
    );
  }

  url: string = 'https://raw.githubusercontent.com/ginkgo-project/ginkgo-data/master/data';
