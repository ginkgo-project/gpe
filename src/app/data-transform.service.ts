...
export class DataTransformService {
  ...
  getTransformExpression(url: string, expressionName: string): Observable<TransformExpression> {
    return this.getTransformExpressionList(url).pipe(
      flatMap((expressionList) => {
        const expression = expressionList.find((e) => e.name === expressionName);
        ...
        return this.http.get(`${url}/${expression.file}`, { responseType: 'text' }).pipe(
          retry(3),
  ...
  private getTransformExpressionList(url: string): Observable<TransformExpression[]> {
    const databaseEntry = this.expressionDatabase.get(url);
    if (databaseEntry) {
